...
|---|---|---|
| `hooks/arm-nudge.mjs` | `UserPromptSubmit` | On a leaving phrase ("me voy a dormir", "vuelvo en 2 horas", "afk"...), creates the state dir, writes `owner` with the session id and injects the arming order, so the ladder goes up in that same turn. The phrase is looked for only in what the user wrote: fenced code blocks and `>` quoted lines are skipped, and a message longer than 280 characters counts only by its last sentence. "buenas noches", "hasta mañana", "afk" and "brb" count only when they close the message (a duration may follow them: "afk 2h", "brb 10 min"), "vuelvo en" needs a time ("vuelvo en 2 horas", "vuelvo en 20 min", "vuelvo en 1h", "vuelvo en la tarde") and "cuando vuelvo..." is not an announcement. On any other message, once the ladder is older than 10 minutes, it writes `stop` itself: the user is back. Background-task wakes (`KEEPALIVE_TICK`, `KEEPALIVE_STOPPED`, finished jobs and workflows) also fire this hook as prompts starting with `<task-notification>`; those are never read as a leaving phrase or a return. It also reports a ladder that died while they were away, even on a notification. When every rung went stale and no `DEAD` marker exists (a window reload kills the rungs and no turn ends, so the watchdog never runs), the user's return stops that ladder and reports that it died and since when, instead of counting remaining rungs. Arming clears the old `.alive` files, so a re-armed ladder keeps its grace. |
